...
**「フレームワーク」** = 家を建てるときの設計図や骨組みのようなもの

**Next.jsの構造:**
- **フロントエンド(見た目)**: Reactで作ります
- **バックエンド(裏側の処理)**: Node.jsで動きます

**Next.jsには4つのページ生成方法があります:**

| 方法 | いつHTMLが作られる？ | わかりやすく言うと | 使いどころ |
|------|---------------------|-------------------|------------|
| **CSR** | ユーザーのブラウザで | 「あとで作る」方式。最初は空っぽ、JavaScriptで中身を入れる(SPA) | ログイン後の管理画面、会員専用ページ |
| **SSR** | ユーザーがアクセスした瞬間 | 「その場で作る」方式。新鮮な情報をすぐ表示 | 最新ニュース、株価、天気予報、商品詳細 |
| **SSG** | サイトを公開する前 | 「事前に作っておく」方式。超高速で表示（CDN） | 会社紹介ページ、ブログ記事、ランディングページ |
| **ISR** | 事前に作る+定期的に更新 | 「事前に作るけど、たまに更新」方式 | 商品一覧(在庫が変わる)、人気記事ランキング |
...
| サービス名 | 何をするもの？ | アクセス先 |
|-----------|---------------|-----------|
| **web** | Next.js(あなたが作るWebアプリ) | http://localhost:3000 |
| **db** | MySQL(データを保存する場所) | localhost:3253 |
| **phpmyadmin** | MySQLを見やすく管理するツール | http://localhost:8022 |
...
**Vue.js(ビュージェイエス)** は、Reactと同じく **UIライブラリ** です。
