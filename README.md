...
案件に伴い、勉強用に作成しました。
メモ帳替わりに使用してるので、クオリティの低さはご了承ください。
...
## 🫛 Vue.jsとNuxt.jsって見たことあるけどこれは何だ？

**Vue.jsはReactと同じでライブラリUI**ではありますが、小さいフレームワーク機能もあります。  
Vue.jsが対応してるフレームワークが**Nuxt.js**（読み:ナクスト）です。誤字ではありません...ややこしい...
まとめると...  
React → Next.js  
Vue → Nuxt.js  
がセットで使われます。どちらもTypeScriptが使えますが、考え方は一緒で無くても開発できます。  

NextもNuxtもなんでこんなややこしい名前なのか?...不思議で調べてみました。  
開発者たちは「Vue版Next.jsを作ろう！」って発想で名前を付けたので、  
Next は React の Next、  
Nuxt は Vue の Next（N + Vue = Nuxt）  
つまり、「React → Next.js → Vue.js → Nuxt.js」という順で出てきたました。  
ここまでみるとNuxt.js最後だから強そう！って思う方多いと思います（私もです）。  
でも最初に出たNext.jsのほうが自由度が高くて周辺ツールも超充実してるので、大手ではこっちが採用されています（つまり情報が山ほどある）。  
まとめると、vue.js/Nuxt.js は React/Next.js の“良いところ”を見てから作られた軽くて使いやすいフレームワークです。  
Windowsも最初に出てたくさんの機能ありますもんね～。長年洗練されてきた技術は簡単に超えられないということですかね（個人的な感想）。
