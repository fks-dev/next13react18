...
### ⚡**Next.js は “４つからどれか１つを選ぶ” んじゃなくて、ページごと・機能ごとに “全部ミックス” できるフレームワークです（ハイブリッドレンダリング）。 現在のところ、4つを一気に実現できるフレームワークは*Next.js以外存在しません。**  
...
- `useEffect` = ページ読み込み時に1回だけ実行、など発火タイミングを指定できる。
- `useRef` = 更新しない値 or DOMを触るときに使用。
- `useContext` = 深い階層にデータを渡す。propsバケツリレーが不要になる。
...
**13.5以降からは書き直しレベルの変更はほとんどない** とのことなのでご安心ください。  
気になる方は調べてみてください。
